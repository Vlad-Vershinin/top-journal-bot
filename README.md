...
- приватная статистика команд по Telegram user ID;
...
| `STATS_FILE` | — | `data/request_stats.json` | Файл статистики команд |
...
Счётчики команд дополнительно сохраняются в `data/request_stats.json`: user ID,
актуальный username и имя из Telegram, время первого/последнего запроса и число
